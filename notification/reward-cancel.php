...
namespace Sejoli_Reward\Notification;

use Carbon_Fields\Container;
use Carbon_Fields\Field;

class RewardCancel extends \SejoliSA\Notification\Main {
     ...
     * Hooked via filter, sejoli/notification/fields priority 1223
     * @since   1.0.0
     * @param   array $fields All fields for notification setting form
     */
    public function add_setting_fields(array $fields) {

        $fields['reward-cancel'] = [
			'title'  => __('Pembatalan Penukaran Hadiah', 'sejoli'),
			'fields' => [

                // Untuk buyer
				Field::make('separator'	,'sep_cancel_exchange_email', 	__('Email' ,'sejoli'))
				...
				Field::make('rich_text', 'cancel_exchange_email_content', __('Konten', 'sejoli'))
					->set_required(true)
					->set_default_value(sejoli_get_notification_content('cancel-exchange-user')),

				Field::make('separator'	,'sep_cancel_exchange_sms', 	__('SMS' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media sms', 'sejoli')),

				Field::make('textarea', 'cancel_exchange_sms_content', __('Konten', 'sejoli'))
                    ->set_default_value(sejoli_get_notification_content('cancel-exchange-user', 'sms')),

				Field::make('separator'	,'sep_cancel_exchange_whatsapp', 	__('WhatsApp' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media whatsapp', 'sejoli')),

				Field::make('textarea', 'cancel_exchange_whatsapp_content', __('Konten', 'sejoli'))
                    ->set_default_value(sejoli_get_notification_content('cancel-exchange-user', 'whatsapp')),

                // Untuk admin
				Field::make('separator', 	'sep_cancel_exchange_admin', 		__('Konten untuk admin dan lainnya','sejoli'))
					->set_classes('main-title'),

				Field::make('checkbox',		'cancel_exchange_admin_active',	__('Aktifkan notifikasi untuk admin', 'sejoli'))
                    ->set_default_value(true),

				Field::make('separator',	'sep_cancel_exchange_admin_email', 	__('Email' ,'sejoli'))
				...
				Field::make('text',		'cancel_exchange_admin_email_recipient', __('Alamat Email Penerima', 'sejoli'))
				...
				Field::make('rich_text', 'cancel_exchange_admin_email_content', __('Konten', 'sejoli'))
					->set_required(true)
					->set_default_value(sejoli_get_notification_content('cancel-exchange-admin'))
					->set_conditional_logic([
						[
							'field'	=> 'cancel_exchange_admin_active',
							'value' => true,
				...
				Field::make('separator'	,'sep_cancel_exchange_admin_sms', 	__('SMS' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media sms', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'cancel_exchange_admin_active',
				...
				Field::make('text',		'cancel_exchange_admin_sms_recipient', __('Nomor SMS Penerima', 'sejoli'))
					->set_help_text(__('Gunakan tanda koma jika penerima ada lebih dari 1', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'cancel_exchange_admin_active',
				...
				Field::make('textarea', 'cancel_exchange_admin_sms_content', __('Konten', 'sejoli'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via sms'))
					->set_default_value(sejoli_get_notification_content('cancel-exchange-admin', 'sms'))
					->set_conditional_logic([
						[
							'field'	=> 'cancel_exchange_admin_active',
							'value' => true,
				...
				Field::make('separator'	,'sep_cancel_exchange_admin_whatsapp', 	__('WhatsApp' ,'sejoli'))
					->set_help_text(__('Pengaturan konten untuk media whatsapp', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'cancel_exchange_admin_active',
				...
				Field::make('text',		'cancel_exchange_admin_whatsapp_recipient', __('Nomor WhatsApp Penerima', 'sejoli'))
					->set_help_text(__('Gunakan tanda koma jika penerima ada lebih dari 1', 'sejoli'))
					->set_conditional_logic([
						[
							'field'	=> 'cancel_exchange_admin_active',
				...
				Field::make('textarea', 'cancel_exchange_admin_whatsapp_content', __('Konten', 'sejoli'))
					->set_help_text(__('Dengan mengosongkan isian ini, tidak akan ada notifikasi yang dikirimkan via whatsapp'))
					->set_default_value(sejoli_get_notification_content('cancel-exchange-admin', 'whatsapp'))
					->set_conditional_logic([
						[
							'field'	=> 'cancel_exchange_admin_active',
							'value' => true,
    ...
    protected function set_content() {
		...
		$this->set_recipient_title  ('buyer', 'email', carbon_get_theme_option('cancel_exchange_email_title'));
		$this->set_recipient_content('buyer', 'email', $this->set_notification_content(
												carbon_get_theme_option('cancel_exchange_email_content'),
												'email',
                                                'buyer'
											 ));

		if(!empty(carbon_get_theme_option('cancel_exchange_whatsapp_content'))) :

            $this->set_enable_send('whatsapp', 'buyer', true);
			$this->set_recipient_content('buyer', 'whatsapp', $this->set_notification_content(
		                                                carbon_get_theme_option('cancel_exchange_whatsapp_content'),
		                                                'whatsapp',
                                                        'buyer'
                                                    ));
        endif;

		if(!empty(carbon_get_theme_option('cancel_exchange_sms_content'))) :
            $this->set_enable_send('sms', 'buyer', true);
			$this->set_recipient_content('buyer', 'sms', $this->set_notification_content(
                                    				carbon_get_theme_option('cancel_exchange_sms_content'),
                                    				'sms',
                                                    'buyer'
                                    			));
        endif;
		...
		if(false !== carbon_get_theme_option('cancel_exchange_admin_active')) :

            $this->set_enable_send('email', 'admin', true);
    		$this->set_recipient_title('admin', 'email', carbon_get_theme_option('cancel_exchange_admin_email_title'));
    		$this->set_recipient_content('admin', 'email', $this->set_notification_content(
													carbon_get_theme_option('cancel_exchange_admin_email_content'),
													'email',
                                                    'admin'
												));

    		if(!empty(carbon_get_theme_option('cancel_exchange_admin_whatsapp_content'))) :

				$this->set_enable_send('whatsapp', 'admin', true);
				$this->set_recipient_content('admin', 'whatsapp', $this->set_notification_content(
                                                			carbon_get_theme_option('cancel_exchange_admin_whatsapp_content'),
                                                			'whatsapp',
                                                            'admin'
                                                		));
            endif;

    		if(!empty(carbon_get_theme_option('cancel_exchange_admin_sms_content'))) :
                $this->set_enable_send('sms', 'admin', true);
				$this->set_recipient_content('admin', 'sms', $this->set_notification_content(
                                        				carbon_get_theme_option('cancel_exchange_admin_sms_content'),
                                        				'sms',
                                                        'admin'
                                        			));
            endif;
    ...
    protected function check_recipients($media = 'email', $role = 'admin') {
        ...
        $recipients       = carbon_get_theme_option('cancel_exchange_' . $role . '_' . $media . '_recipient');
